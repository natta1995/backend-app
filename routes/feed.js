...
router.get("/", (req, res) => {
  ...
  const query = `
        SELECT posts.id, posts.content, posts.createdAt, posts.image_url, users.username, users.profile_image, posts.likes 
        FROM feed posts 
...
router.post("/create", upload.single("image"), (req, res) => {
  ...
  if (!req.session.userId) {
    return res
      .status(401)
      .send("Du måste vara inloggad för att skapa ett inlägg.");
  }
  ...
  const insertQuery =
    "INSERT INTO feed (userId, content, image_url) VALUES (?, ?, ?)";
  db.execute(
    insertQuery,
...
router.post("/:feedId/like", (req, res) => {
  ...
  if (!req.session.userId) {
    return res
      .status(401)
      .send("Du måste vara inloggad för att gilla ett inlägg.");
  }
...
router.delete("/:id", (req, res) => {
  ...
  if (!req.session.userId) {
    return res
      .status(401)
      .send("Du måste vara inloggad för att ta bort ett inlägg.");
  }
  ...
  db.execute(query, [postId, req.session.userId], (err, results) => {
    ...
    if (results.length === 0) {
      return res
        .status(403)
        .send("Du har inte behörighet att ta bort detta inlägg.");
    }
...
router.get("/:feedId/comments", (req, res) => {
  ...
  const query =
    "SELECT comments.*, users.username, users.profile_image FROM comments JOIN users ON comments.user_id = users.id WHERE post_id = ? ORDER BY comments.created_at DESC";
  db.execute(query, [feedId], (err, results) => {
    if (err) {
...
router.post("/:feedId/comments", (req, res) => {
  ...
  db.execute(
    insertQuery,
    [feedId, req.session.userId, content],
    (err, result) => {
      if (err) {
...
router.delete("/comments/:commentId", (req, res) => {
  ...
  if (!req.session.userId) {
    return res
      .status(401)
      .send("Du måste vara inloggad för att ta bort en kommentar.");
  }
  ...
  db.execute(query, [commentId, req.session.userId], (err, results) => {
    ...
    if (results.length === 0) {
      return res
        .status(403)
        .send("Du har inte behörighet att ta bort denna kommentar.");
    }
